...
/* ── Constants ──────────────────────────────────────────────── */
const TITLE = "ประวัติการให้บริการข้อมูล อท. (License Logs)";
const SUBTITLE = "ข้อมูลประวัติการให้บริการข้อมูลใบอนุญาต/หนังสืออนุญาต ของ กรมการอุตสาหกรรมทหาร บน LinkageII";
const CHART_TITLE = "License Request";
const CHART_COLOR = "#003087";
const SEARCH_PLACEHOLDER = "ค้นหา Log ID, รหัสหน่วยงาน, บริการ...";
...
const sids = ["DOTI-0001", "DOTI-0002", "DOTI-0004", "DOTI-0007", "DOTI-0009"];
const serviceW = [38, 29, 22, 7, 4];
...
const times = ["55ms","72ms","88ms","112ms","134ms","178ms","201ms","256ms","312ms","445ms"];
...
/* ── Pinned samples (always row 1-2, within default 7-day range) */
const failureSample: LogEntry = {
  id: "LIC-09999",
  timestamp: "26 พ.ค. 2569, 07:55 น.",
  agencyCode: "0105543000011",
  service: "ใบอนุญาตสั่งเข้ามาซึ่งยุทธภัณฑ์ (ยภ.2)",
  serviceId: "DOTI-0001",
  statusCode: 403,
  responseTime: "201ms",
  ipAddress: "10.201.20.99",
  requestBody: `{"agencyCode":"0105543000011","licenseType":"ORDER_IN","idNumber":"9999999","serviceId":"DOTI-0001"}`,
  responseBody: `{"status":"error","code":403,"message":"Access denied – insufficient permission"}`,
};
const successSample: LogEntry = {
  id: "LIC-09998",
  timestamp: "26 พ.ค. 2569, 08:10 น.",
  agencyCode: "0107546000022",
  service: "ใบอนุญาตนำเข้ามาซึ่งยุทธภัณฑ์ (ยภ.3)",
  serviceId: "DOTI-0002",
  statusCode: 200,
  responseTime: "88ms",
  ipAddress: "10.201.21.55",
  requestBody: `{"agencyCode":"0107546000022","licenseType":"IMPORT","idNumber":"9999998","serviceId":"DOTI-0002"}`,
  responseBody: `{"status":"success","data":{"licenseNo":"WPN-2024001","type":"ใบอนุญาตนำเข้ามาซึ่งยุทธภัณฑ์ (ยภ.3)","holder":"นาย ตัวอย่าง 1","expiry":"2026-12-31","status":"valid"}}`,
};

/* ── Generate logs ──────────────────────────────────────────── */
const generated: LogEntry[] = Array.from({ length: 650 }, (_, i) => {
  const agencyCode = pick(agencyCodes, agencyW, i * 5 + 1);
  const si = services.indexOf(pick(services, serviceW, i * 11 + 3));
  const code = statusPool[(lcg(i * 17 + 7) * statusPool.length) | 0];
  ...
  const licType = si === 0 ? "ORDER_IN" : si === 1 ? "IMPORT" : si === 2 ? "POSSESS" : si === 3 ? "RAW_IMPORT" : "SELL_OUT";
  const ai = agencyCodes.indexOf(agencyCode);
  return {
    ...
    statusCode: code,
    responseTime: time,
    ipAddress: `10.201.${20 + ai}.${50 + (i % 40)}`,
    requestBody: `{"agencyCode":"${agencyCode}","licenseType":"${licType}","idNumber":"${1234567 + i}","serviceId":"${sids[si]}"}`,
    responseBody:
      code === 200
        ...
        : `{"status":"error","code":${code},"message":"${
            code === 404 ? "License record not found"
            : code === 403 ? "Access denied – insufficient permission"
            : "Service temporarily unavailable"
          }"}`,
  };
});
const logs: LogEntry[] = [failureSample, successSample, ...generated];
...
export function LogLicense() {
  ...
  const applyFilters = () => {
    setSearch(dSearch); setServiceFilter(dService);
    setStatusFilter(dStatus); setDateFrom(dFrom); setDateTo(dTo);
  };
  const clearFilters = () => {
    setDSearch(""); setDService("บริการ ทั้งหมด");
    setDStatus("สถานะ ทั้งหมด"); setDFrom(""); setDTo("");
    setSearch(""); setServiceFilter("บริการ ทั้งหมด");
    setStatusFilter("สถานะ ทั้งหมด"); setDateFrom(""); setDateTo("");
  };

  const filtered = logs.filter((l) => {
    const q = search.toLowerCase();
    const matchQ = !q || l.id.toLowerCase().includes(q) || l.agencyCode.includes(q) || l.service.toLowerCase().includes(q);
    ...
    return matchQ && matchS && matchSt && (!dateFrom || entryDate >= dateFrom) && (!dateTo || entryDate <= dateTo);
  });
  ...
  const columns: Column<LogEntry>[] = [
    ...
    {
      key: "agencyCode", header: "รหัสหน่วยงาน", width: "160px",
      render: (row) => (
        <span style={{ fontFamily: "monospace", fontSize: "13px", fontWeight: 400, color: "#374151" }}>
          {row.agencyCode}
        </span>
      ),
    },
  ...
  return (
    <div>
      ...
      <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "18px 24px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.05)", marginBottom: "16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr auto auto", columnGap: "12px" }}>
          ...
          {/* สถานะ spans cols 3-4 → same total width as the two buttons below */}
          <div style={{ gridColumn: "3 / 5", display: "flex", flexDirection: "column", gap: "5px", paddingBottom: "14px" }}>
          ...
          {/* ── Separator ── */}
          <div style={{ gridColumn: "1 / 5", height: "1px", backgroundColor: "#F3F4F6", marginBottom: "14px" }} />

          {/* ── Row 2: search + buttons ── */}
          {/* Search spans cols 1-2 → same total width as the two filter cells above */}
      ...
      {/* Detail Modal */}
      <Modal open={!!selectedLog} onClose={() => setSelectedLog(null)} title="รายละเอียด Log" subtitle="การให้บริการข้อมูลใบอนุญาต/หนังสืออนุญาต ของ กรมการอุตสาหกรรมทหาร" size="lg"
        icon={<FileSearch size={17} color={CHART_COLOR} />} iconBg="#EEF2FF"
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
              {([
                ...
                ["รหัสหน่วยงาน", selectedLog.agencyCode],
                ...
                <div key={label} style={{ backgroundColor: "#F9FAFB", borderRadius: "8px", padding: "10px 12px" }}>
                  ...
                  <div style={{ fontSize: "13px", color: "#111827", fontFamily: ["รหัสตอบกลับ","Response Time","IP Address","Log ID","รหัสหน่วยงาน"].includes(label) ? "monospace" : FF, fontWeight: 400 }}>{value}</div>
                </div>
              ))}
            </div>
