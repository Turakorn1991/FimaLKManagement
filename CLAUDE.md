...
**สถานะ pill** (all 3 log pages) — `height "24px"` `padding "0 9px"` `minWidth "82px"` `borderRadius "20px"` `inline-flex + alignItems center + justifyContent center`:
- สำเร็จ: bg `#ECFDF5`, color `#059669`, border `#A7F3D0`
- ไม่สำเร็จ: bg `#FEF2F2`, color `#DC2626`, border `#FECACA`

**รหัสตอบกลับ badge** — `height "24px"` `padding "0 9px"` `borderRadius "20px"` `fontSize 12px` `fontWeight 700` monospace `inline-flex + alignItems center + justifyContent center`:
- 2xx: bg `#ECFDF5`, color `#059669`
- 4xx: bg `#FFFBEB`, color `#D97706`
- 5xx: bg `#FEF2F2`, color `#DC2626`

Both badges use `height: "24px"` with `padding: "0 9px"` (no vertical padding) so they are always equal height regardless of font size difference.

**IP Address** column: always header **"IP Address"** (not "IP"), width `140px`, monospace, `fontWeight 400`

**Log page column fontWeight rule** — all text data `fontWeight: 400`. Exceptions that keep bold:
- สถานะ pill: `fontWeight 600`
- รหัสตอบกลับ badge: `fontWeight 700`

**LOG ID column** (all 3 log pages): `fontSize 12px`, `fontWeight 400`, monospace

**วัน-เวลา column** (all 3 log pages): custom render — `fontSize 13px`, `fontWeight 400`, `fontFamily "Inter, sans-serif"`, `whiteSpace "nowrap"`
...
**Columns** — LOG ID (130px, monospace, `fontSize 12px`, `fontWeight 400`) → วัน-เวลา (185px, sortable, `fontWeight 400`) → แอปพลิเคชัน (auto, name `fontWeight 400` + clientId sub-text `fontSize 10px` monospace) → บริการ (auto, name `fontWeight 400` + serviceId sub-text) → สถานะ (118px) → รหัสตอบกลับ (90px) → IP Address (140px, `fontWeight 400`) → # (44px)
...
**Columns** — LOG ID (130px, monospace, `fontSize 12px`, `fontWeight 400`) → วัน-เวลา (185px, sortable, `fontWeight 400`) → รหัสหน่วยงาน (160px, monospace, 13-digit, `fontWeight 400`) → บริการ (auto, name `fontWeight 400` + serviceId sub-text) → สถานะ (118px) → รหัสตอบกลับ (90px) → IP Address (140px, `fontWeight 400`) → # (44px)
...
**`actionConfig`** — used only for filter dropdown `Object.keys(actionConfig)`, NOT for rendering:
```ts
CREATE | UPDATE | DELETE | LOGIN | REVOKE | GRANT
```

**Columns** — LOG ID (120px, monospace, `fontSize 12px`, `fontWeight 400`) → วัน-เวลา (185px, sortable, `fontWeight 400`) → ผู้ดำเนินการ (auto, `fontWeight 400`) → การกระทำ (110px, center, **plain text** `fontSize 13px` `fontWeight 400` color `#111827` — no badge) → เหตุการณ์ (auto, entity name `fontWeight 400` + `"RefId : {entityId}"` sub-text `fontSize 11px` monospace `#6B7280`) → สถานะ (118px, green/red pill) → รหัสตอบกลับ (90px, derived) → IP Address (140px, `fontWeight 400`) → # (44px, Eye — ALL rows)
...
interface ClientPerm {
  permissionId: string; clientId: string; clientName: string; department: string;
...
**Data** — `initClients`: 8 hardcoded `ClientPerm` entries (`CLT-001`…`CLT-008`) with `permissionId: "PRM-001"`…`"PRM-008"`. `ALL_SERVICES`: 22 services. `CLIENT_OPTIONS`: 8 client options for dropdown. New entries created via `handleSave` auto-generate `permissionId: "PRM-{nextNum}"`.

**Main table columns** — Permission ID (120px, monospace, `fontWeight 400`, shows `row.permissionId` only — no sub-text) → แอปพลิเคชัน (auto, name + department sub-text) → จำนวนบริการ (180px, center, indigo pill button opens view modal) → อัปเดตล่าสุด (200px, date + "โดย" sub-text) → จัดการ (120px, Edit + Delete icon buttons)

**Search** — `width: "75%"`, placeholder `"ค้นหา Permission ID, ชื่อแอปพลิเคชัน, หน่วยงาน..."`, filter includes `permissionId`, `clientId`, `clientName`, `department`

**PermissionModal** (inline component, not using shared `Modal`):
- Size: `maxWidth: 820px`, `maxHeight: 90vh`, scrollable body
- Header icon: **ALL modes** use `ShieldCheck` navy `#003087` / bg `#EEF2FF` (no purple Eye variant)
- Header subtitle (non-create): `"Permission ID: ${editClient?.permissionId}"`
...
- **Footer (view mode)**: `borderTop: "1px solid #F3F4F6"`, `backgroundColor: "white"` — matches shared `Modal` component footer. ปิด button: `<Button onClick={onClose}>ปิด</Button>` (primary, navy — no variant prop)
- **Footer (edit/create mode)**: `borderTop: "1px solid #E5E7EB"`, `backgroundColor: "#FAFAFA"`. ยกเลิก: `<Button variant="secondary">`, บันทึก: `<Button icon={Save} onClick={() => setShowSaveConfirm(true)}>` (disabled if create + no client, bg `#CBD5E1`)
- **Save ConfirmDialog** (`showSaveConfirm`) — `variant="success"`, `confirmLabel="ยืนยัน"`:
  - Create: `title="ยืนยันการเพิ่มข้อมูล"` · `message="คุณต้องการเพิ่มข้อมูลหรือไม่?"`
  - Edit: `title="ยืนยันการแก้ไขข้อมูล"` · `message="คุณต้องการแก้ไขข้อมูลหรือไม่?"`
...
- Fixed top bar, height 64px, navy `#003087` background
- Left: hamburger (`Menu`) toggle → calls `onToggleSidebar`, Shield logo + title
- Right: user menu button (`showUserMenu` toggle) — avatar + name + `ChevronDown`
- **Click-outside**: `useRef<HTMLDivElement>` on wrapper div + `useEffect` adds `mousedown` listener on document when `showUserMenu === true`; clicks outside the ref close the dropdown. Listener is removed on cleanup.
- User menu dropdown: single item "ออกจากระบบ / Sign out", no action wired (mock)
